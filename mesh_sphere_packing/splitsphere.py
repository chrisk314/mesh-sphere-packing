...
def output_segments_geomview(segments):
    for i, (tris, points) in enumerate(segments):
...
def translate_segments(segments, Lx, Ly, Lz):
    ...
    for (tris, points), t in zip(segments, translations):
        points += t
    return segments
...
def triangulate_segment_points(point_sets, x, y, z):
    ...
    def extract_surf_from_chull(chull, x, y, z):
        ...
        # TODO : Find a cleaner/simpler/more elegant way of extractin sphere triangles
        tri_norms = get_tri_norms(chull.simplices, chull.points)
        tri_pos_norms = get_tri_centroids_norm(chull.simplices, chull.points, sphere_center)
...
def triangulate_segment_points_cgal_alt(point_sets):
    ...
    for points in point_sets:
        points_cgal = [Point_3(*x) for x in points]
        poly = Polyhedron_3()
        CGAL_Convex_hull_3.convex_hull_3(points_cgal, poly)
        res.append(poly)
    return res
...
if __name__ == '__main__':
    # x, y, z, r, Lx, Ly, Lz = get_args(sys.argv)
    x, y, z, r, Lx, Ly, Lz = 0., 0., 0., 0.5, 2., 2., 2.
    #points = gen_sphere_hypercube_points(x, y, z, r)
    ...
    segments = translate_segments(segments, Lx, Ly, Lz)
